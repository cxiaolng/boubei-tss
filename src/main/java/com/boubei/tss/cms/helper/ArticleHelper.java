...
public class ArticleHelper {
	 ...
	 * @return
	 */
	public static String getArticlePublishPath(Date createTime) {
	    if(createTime == null) {
	        createTime = new Date();
	    }
    ...
    public static void addPicListInfo(Element itemElement, Collection<Attachment> attchments){
        for ( Attachment attachment : attchments ) {
            Element attachmenetElement = itemElement.addElement("Attachment"); // 在指定节点下为每个附件增加一个Attachment节点
            if (attachment.isImage()) {
                attachmenetElement.addAttribute("type", "image");
            } 
            else if (attachment.isOfficeDoc()) {
