package com.boubei.tss.framework.component.progress;
...
public class Progress {
    ...
    private void setCompleted(long completed) {
        ...
        if (curComplete != 0) {
            ...
            if (delay > MAX_LOAD_SCHEDULE_TIME) {
                this.delay = MAX_LOAD_SCHEDULE_TIME;
            }
            else if (delay < DEFAULT_LOAD_SCHEDULE_TIME) {
                this.delay = DEFAULT_LOAD_SCHEDULE_TIME;
            } 
            else {
                this.delay = delay;
